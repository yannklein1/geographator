...
let nombreAleatoire = Math.floor(Math.random()*(listePaysCapitalesMonde.length))

function melange () {
	nombreAleatoire = Math.floor(Math.random()*(listePaysCapitalesMonde.length))
}
...
function initialisation () {
	...
	zoneQuestion.innerHTML = listePaysCapitalesMonde[nombreAleatoire].pays
...
function calculScore () {
	...
	let bonneReponse = suppressionAccents(listePaysCapitalesMonde[nombreAleatoire].capitale.toLowerCase())
...
function affichageMauvaiseReponse () {
	zoneReponsePrecedente.innerHTML = "Mauvaise réponse, la réponse était " + listePaysCapitalesMonde[nombreAleatoire].capitale
}
