...
let nombreAleatoire = Math.floor(Math.random()*(listePaysCapitalesAsie.length))

function melange () {
	nombreAleatoire = Math.floor(Math.random()*(listePaysCapitalesAsie.length))
}
...
function initialisation () {
	...
	zoneQuestion.innerHTML = listePaysCapitalesAsie[nombreAleatoire].pays
...
function calculScore () {
	...
	let bonneReponse = suppressionAccents(listePaysCapitalesAsie[nombreAleatoire].capitale.toLowerCase())
...
function affichageMauvaiseReponse () {
	zoneReponsePrecedente.innerHTML = "Mauvaise réponse, la réponse était " + listePaysCapitalesAsie[nombreAleatoire].capitale
}
